...
export class RFQController {
    ...
    async addSupplierQuote(req, res) {
        try {
            const { id } = req.params;
            const quote = req.body;
            if (!id || !quote)
                return res.status(400).json({ error: "Missing rfq id or quote payload" });
    ...
    async updateRFQ(req, res) {
        try {
            ...
            if (!id)
                return res.status(400).json({ error: "Missing rfq id" });
    ...
    async awardRFQ(req, res) {
        try {
            ...
            if (!id || !targetVendor)
                return res.status(400).json({ error: "Missing rfq id or vendor name" });
