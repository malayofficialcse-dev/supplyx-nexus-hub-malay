...
export class RequisitionService {
  ...
  async createRequisition(data: {
    ...
    try {
      ...
      if (budget) await (operationsService as any).reserveBudget({ budgetId: budget.id, requisitionId: newReq.id, amount: data.total }, data.requester);
    } catch (budgetError) {
  ...
  async approveRequisition(
    id: string,
    approverData: { name: string; role?: string; decision?: string; notes?: string }
  ): Promise<Requisition> {
    ...
    const existingApprovals = Array.isArray(existing.approvals) ? (existing.approvals as any[]) : [];
